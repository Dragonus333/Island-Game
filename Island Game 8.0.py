...
class Land():
    """A hexagon tile"""
    def __init__(self,x,y,z,type="Plains",structure=None):
    ...
    def draw(self):
        ...
        canvas.create_polygon(hex_corner_hexCoords,fill=colour[1],outline=colour[0],tags=[str(self.hexCoords)])
    ...
    def destroy(self):
        if not self.structure == "Road":
            canvas.delete(canvas.find_withtag("letter "+str(self.x+self.y)))
    ...
    def link(self,other_land,link_type="Road"):
        if link_type == "Road":
            canvas.create_line(self.x,self.y,other_land.x,other_land.y,fill="black",dash=(1,4),tags=["Road "+str(self.x+self.y)])
    ...
    def hightlight(self,hightlight=True):
        if hightlight:
            canvas.itemconfig(canvas.find_withtag(self),outline="blue")
        else:
            if self.type == "Lake":
                canvas.itemconfig(canvas.find_withtag(self),outline="powder blue")
            elif self.type == "Shallow Waters":
                canvas.itemconfig(canvas.find_withtag(self),outline="light blue")
            else:
                try:
...
class Settlement():
    ...
    def collect_resources(self):
        ...
        for land in self.land.find_nearby(self.range_of_collection): #search nearby lands     
                for resource in land.resources:
                    if land.farming_type == "Sustainable": #if farmining is sustainable
                        collected_resource = land.resources[resource]
                    elif land.farming_type == "Unsustainable": #if farmining is sustainable
                        collected_resource = land.resources[resource] * 2
                    elif land.farming_type == "Growth": #if farmining is sustainable
    ...
    def manufacture(self):
        ...
        if "Forge" in self.special_buildings:
            try:
                ...
                try:
                    self.stored_resources["steel"] += int(self.stored_resources["wheat"] - 5 / 5)
                except KeyError:
                    self.stored_resources["steel"] = int(self.stored_resources["wheat"] - 5 / 5)
    ...
    def send_resources(self,resources,to,send_error=True):
        ...
        if path_found == True:
            ...
            for item in resources:
                try:
                    to.stored_resources[item] += resources[item]
                except KeyError:
                    to.stored_resources[item] = resources[item]
...
def save_world(save_mode="save"):
    ...
    if save_mode == "saveAs":
        ...
        try:
            world_file = open(World_Name + ".wor","r")
        except FileNotFoundError:
            world_file = asksaveasfile(mode='w',initialfile=World_Name,defaultextension=".wor")
            
    for land in Lands:
        x,y,z = land.hexCoords
        world_file.write(str(land.hexCoords)+"!"+land.type+"!"+str(land.structure)+"!\n")
    world_file.close()
...
def click(event,twoclicks=False):
    ...
    if canvas.find_withtag(CURRENT): #if user has clicked object on canvas
        ...
        try:
            canvas.itemconfig(LAST, outline="black") #turn last object clicked back to red
        except:
            pass #their is no last object 
        ...
        info("User clicked")
        try:
            if canvas.gettags(LAST)[0].type == "Lake":
                canvas.itemconfig(LAST, outline="powder blue")
            elif self.type == "Shallow Waters":
                canvas.itemconfig(LAST,outline="light blue")
            else:
                canvas.itemconfig(LAST, outline="black")
        except:
            try:
                canvas.itemconfig(LAST, outline="black")
            except TclError:
...
def update_screen(object_class=None,info_about="World"):
    ...
    if show_owned_land.get() == True:
         for land in Lands:
            if land.owned == True:
                land.hightlight()
    else:
        for land in Lands:
            if not land.selected:
                land.hightlight(False)
